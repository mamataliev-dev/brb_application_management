...
def test_update_manager_as_admin(client, test_app, sample_admin, sample_manager):
    """Test that an admin can update a manager's details."""
    with test_app.test_request_context():
        session["user"] = {"role": "admin"}

        query = """
        mutation {
          updateManager(input: {id: "1", name: "Updated Name", branch: "New Branch"}) {
            manager {
              name
              branch
            }
        ...
        data = response.get("data", {}).get("updateManager", {}).get("manager", {})
...
def test_update_manager_as_non_admin(client, test_app, sample_manager):
    """Test that a non-admin user cannot update a manager's details."""
    with test_app.test_request_context():
        session["user"] = {"role": "manager"}

        query = """
        mutation {
          updateManager(input: {id: "1", name: "Unauthorized Update"}) {
            manager {
              name
            }
          }
